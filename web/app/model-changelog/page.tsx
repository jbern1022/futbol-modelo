...
function modelLabel(name: string): string {
  if (name.startsWith("slate_generator_")) {
    ...
    return `${LEAGUE_LABELS[key] ?? key} Slate Generator`;
  }
  return name;
}
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black">
      <main className="mx-auto max-w-3xl px-6 py-16">
        ...
        <p className="mt-4 max-w-2xl text-zinc-600 dark:text-zinc-400">
          Every model retrains from scratch on a schedule &mdash; no saved
          checkpoints, no hidden manual tuning. Each row here is a real
          retrain event with the holdout metrics it produced, most recent
          first. This is the honest record of ongoing iteration, including
          runs that didn&apos;t improve anything.
        </p>

        {changelog.length === 0 && (
          <div className="mt-10 rounded-lg border border-zinc-200 bg-white p-6 text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900">
            No retrain history yet &mdash; check back after the next scheduled run.
          </div>
        )}
          ...
          {byModel.map(({ name, rows }) => (
            <section key={name}>
              ...
              <p className="mt-1 text-xs text-zinc-400">
                {rows.length} retrain{rows.length === 1 ? "" : "s"} recorded
              </p>
                ...
                {rows.map((r, i) => (
                  ...
                  >
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      ...
                      <span className="text-xs text-zinc-400">version {r.version_tag}</span>
                    </div>
                    {r.training_window && (
                      <p className="mt-1 text-xs text-zinc-500">
                        Trained on: {r.training_window}
                      </p>
                    )}
