...
function sideLabel(market: string, side: string, t: (key: string) => string): string {
  if (market === "1X2") {
    return { home: t("homeWin"), away: t("awayWin"), draw: t("draw") }[side] ?? side;
  }
...
function ReliabilityDiagram({ calibration }: { calibration: CalibrationRow[] }) {
  const t = useTranslations("CalibrationSection");
  const pad = 44;
  const plot = 360;
  ...
  return (
    <>
      <div className="mt-4 flex items-center justify-between">
        ...
        >
          {bandOnly ? t("showingBandOnly") : t("isolateBand")}
        </button>
      </div>
      ...
      {grouped.length === 0 ? (
        <p className="mt-6 text-sm text-zinc-500">{t("noBandsYet")}</p>
      ) : (
          ...
          {grouped.map(({ key, league, market, side, rows }) => (
            <div key={key} className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-sm font-medium text-black dark:text-zinc-50">
                {marketLabel(market)} &mdash; {sideLabel(market, side, t)}{" "}
                ...
                {rows.map((row, i) => (
                  <div key={i} style={{ opacity: opacityForN(row.n) }}>
                    <div className="text-xs text-zinc-400">
                      {t("confidenceBand", { pct: (row.avg_stated_prob * 100).toFixed(0) })}
                      <span className="ml-1 text-zinc-500">(n={row.n})</span>
                    </div>
                    <div className="mt-1 space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="w-16 text-xs text-zinc-500">{t("stated")}</span>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="w-16 text-xs text-zinc-500">{t("realized")}</span>
                        <div className="h-2 flex-1 rounded-full bg-zinc-100 dark:bg-zinc-800">
