  ...
  return (
    <div className="mt-4">
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-zinc-200 bg-zinc-100 text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900">
            <tr>
              ...
              <th scope="col" className="px-4 py-2 font-medium text-right">{t("matches")}</th>
              <th scope="col" className="px-4 py-2 font-medium text-right">{t("rps")}</th>
              <th scope="col" className="px-4 py-2 font-medium text-right">{t("logLoss")}</th>
              <th scope="col" className="px-4 py-2 font-medium text-right">{t("accuracy1x2")}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
      ...
      <p className="mt-1.5 text-xs text-zinc-500">
        {t("rpsRange", { n: rows.length, min: min.toFixed(4), max: max.toFixed(4) })}
      </p>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black">
      <main className="mx-auto max-w-2xl px-6 py-16">
        ...
        <Link href="/track-record" className="mt-8 inline-block rounded-md border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900">
