...
export default async function BankrollPage() {
  const t = await getTranslations("BankrollPage");
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black">
      <main className="mx-auto max-w-3xl px-6 py-16">
        ...
        <p className="mt-4 max-w-2xl text-zinc-600 dark:text-zinc-400">
          {t("explainer", { stake: bets[0]?.stake?.toFixed(0) ?? "10" })}
        </p>
        ...
        {bets.length === 0 ? (
          ...
          <>
            <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
              <StatCard label={t("betsPlaced")} value={bets.length.toLocaleString("en-US")} />
              <StatCard label={t("totalStaked")} value={`$${totalStake.toFixed(0)}`} />
              <StatCard
                label={t("netProfit")}
                value={`${totalProfit >= 0 ? "+" : ""}$${totalProfit.toFixed(2)}`}
              />
              <StatCard label={t("roi")} value={`${roiPct >= 0 ? "+" : ""}${roiPct.toFixed(1)}%`} />
            </div>
            ...
            {bets.length < 30 && (
              <p className="mt-3 text-xs text-amber-700 dark:text-amber-400">
                &#9888; {t("smallSampleWarning", { n: bets.length })}
              </p>
            )}
                ...
                return (
                  <div key={m} className="flex justify-between border-b border-zinc-100 py-2 dark:border-zinc-800">
                    ...
                    <span className="text-black dark:text-zinc-50">
                      {t("betsCount", { n: marketBets.length })} &middot;{" "}
                      {marketProfit >= 0 ? "+" : ""}
