...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "FixturePage" });
  const data = await getSlate(matchId);
  if (!data) return { title: t("fixtureNotFoundTitle") };

  const { home, away, league } = data.fixture;
  const title = `${home} vs ${away} — Futbol Modelo`;
  const description = t("metaDescription", { league, home, away });
      ...
      <main className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-6 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-zinc-500">
              ...
              />{" "}
              <span className="text-xs">{t("yourLocalTime")}</span>
            </p>
          </div>
          <Link href={`/petey?mode=form&league=${encodeURIComponent(fixture.league)}&team=${encodeURIComponent(fixture.home)}`} className="whitespace-nowrap rounded-md border border-zinc-200 px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900">
            {t("askPetey")}
          </Link>
        </div>
        ...
        <div className="mt-8 space-y-6">
          {predictions.length === 0 && (
            <p className="text-zinc-500">{t("noPredictionsYet")}</p>
          )}
