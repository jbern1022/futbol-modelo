  ...
  return (
    <>
      ...
      <p className="mt-2 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">
        Dixon-Coles fits a home-advantage term (&ldquo;gamma&rdquo;) separately for
        every league and season, using only that season&apos;s finished matches.
        Higher means a bigger edge for the home side that season.
      </p>
      <div className="mt-4 overflow-x-auto rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <svg viewBox={`0 0 ${width} ${height}`} className="mx-auto" style={{ minWidth: Math.min(width, 640) }} role="img" aria-label="Home advantage (gamma) by league and season">
          {[0, 0.125, 0.25, 0.375, 0.5].map((t) => (
            <g key={t}>
              <line x1={pad} y1={toY(t)} x2={pad + plotW} y2={toY(t)} stroke="currentColor" strokeOpacity={0.08} />
        ...
        <p className="mt-2 text-center text-xs text-zinc-400">
          Season labels along the axis follow the first league plotted; hover a
          point for its own league and season.
        </p>
      </div>
    </>
  );
