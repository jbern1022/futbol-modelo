...
| Real bookmaker odds are stored with the bookmaker's vig removed (no-vig probability), MLS only | `fetch_and_store_odds()`, `src/ingestion/api_football.py`, `match_odds` table | **Live-verified** 2026-09-05 — ran `fetch_and_store_odds("MLS", 7)` against the real API-Football endpoint: 28 scheduled MLS fixtures found, 102 odds rows stored/updated across 7 fixtures with odds currently posted (Bet365, William Hill, Marathonbet), confirmed via direct query against `match_odds` |
