...
| Corrections are new `void` grades, never edits to the original row | `prediction_grades` schema + convention, e.g. `sql/void_duplicate_match_predictions.sql` | **Code path only** — the pattern is real and has real precedent, but no test asserts a `prediction_grades` row can't be overwritten (it's a `PRIMARY KEY` on `prediction_id`, not a trigger — a second INSERT fails, but nothing stops a raw `UPDATE prediction_grades` today) |
| `predictions.context` NaN values are sanitized to `null`, not left invalid | `_sanitize_context()`, `src/predictions/generator.py` | **Tested** — `tests/test_prediction_context.py`. Known issue: this test calls `persist_slate()`, which commits internally, so it leaves a real `"test — Corners over 3.5"` row in the live ledger on every run — flagged in Todoist, not fixed here (out of this ticket's scope) |
